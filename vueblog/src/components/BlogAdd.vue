    ...
    export default {
      ...
      methods: {
        async addItem() {
          ...
          try {
            const response = await axios.post(DB_DJANGO.POST, {
            ...
            this.$emit("addItem", response.data); // Если требуется обновить список блоков после создания
          } catch (error) {
            console.log(error);
